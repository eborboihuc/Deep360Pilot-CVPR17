...
args = parse_args()
DOMAIN = args.domain
FEATURE_PATH = cfg.FEATURE_PATH + 'feature_' + DOMAIN + \
    '_' + str(args.n_boxes) + 'boxes/'
LABEL_PATH = './parsed_label/' + DOMAIN + '/'

# test testing batches
batch_paths = glob.glob('batch_data/' + DOMAIN + '/test/batch_clips/*.npy')
n_batches = len(batch_paths)
print n_batches
FEATUREs = [
    'hof', 'divide_area_pruned_boxes', 'pruned_roisavg', 'roislist', 'roisavg',
           'avg_motion', 'avg_flow', 'motion_inclusion']
...
for _ in xrange(5):
    ...
    batch_clips = np.load('batch_data/' + DOMAIN + '/test/batch_clips/batch_{}.npy'.format(batch_id))

    for feature in FEATUREs:
        batch[feature] = np.load('batch_data/' + DOMAIN + '/test/' + feature + \
